/* global d3, $, TreeMap, FlickrUtils, utils, Hammer*/

// TODO Override atob because I disabled base64 on the db
const atob = (d) => d;
...
(function () {
  ...
  hammermc.on("rotatestart rotatemove", function (ev) {
    ...
    dimensions.sort(function (a, b) {
      return (
        Math.abs(ringAngleScale(a) - newAngle) <
        Math.abs(ringAngleScale(b) - newAngle)
      );
    });
  ...
  var svgNavList = d3.select(htmlIDNavList).append("svg");
  var photosLayer = svg
    .append("g")
    .attr("id", "photosLayer")
    .call(zoom)
    .call(drag);
  svg.append("defs").append("g").attr("id", "ring").html(photoRing);
  var ringLayer = svg.append("g").attr("id", "ringLayer").attr("opacity", 0.5);
  ringLayer
    .append("use")
    ...
    .attr(
      "transform",
      "translate(" + ringX() + "," + ringY() + ") scale(" + ringSize() + ") "
    );
  ...
  var navMargin = { top: 0, right: 0, bottom: 0, left: 25 };
  ...
  var callbackPhotos = function (error, mdata) {
    ...
    console.log("------------------------------------");
    console.log(
      " callbackPhotos originalPhotos.length " + originalPhotos.length
    );
  ...
  var callbackPhotosLeft = function (error, mdata) {
    ...
    console.log(
      " callbackPhotosLeft originalPhotos.length " + originalPhotos.length
    );
    updateWithCurrentPhoto();
  ...
  var textX = function (d) {
    ...
    return (
      ...
      (direction === "vertical" ? -1 : 1) *
        r *
        Math.cos((ringAngleScale(d.split(":")[0]) * Math.PI) / 180)
    );
  };
  var textY = function (d) {
    ...
    return (
      ...
      (direction === "vertical" ? 1 : -1) *
        r *
        Math.sin((ringAngleScale(d.split(":")[0]) * Math.PI) / 180)
    );
  };
  var textAnchor = function (d) {
    var dName = d.split(":")[0];
    return ringAngleScale(dName) === 180 || ringAngleScale(dName) === 0
      ? "middle"
  ...
  var yPhoto = function (d, i) {
    ...
    return (
      yScale(Math.floor(pageI / NUM_COLS)) +
  ...
  function updateRing() {
        ...
        d3.select("#ring")
          ...
          .attrTween("transform", function (d, i, a) {
            return d3.interpolateString(
              a,
              "rotate(" +
                ringAngleScale(currentDimension.split(":")[0]) +
                ", 500, 500)"
            );
          });
      });
    ...
    if (!showRing) {
      ringTransition.transition().duration(2000).style("opacity", 0);
    }
    //Text
    var selDimLab = ringLayer.selectAll(".dimensionLabel").data(dimensions);
    selDimLab
      .enter()
      .append("text")
    ...
    selDimLab
      .text(function (d) {
        return d.split(":")[0] + ": " + atob(d.split(":")[1]);
      })
      .attr("x", textX)
      .attr("y", textY)
  ...
  function updateDimensionsOptions(photos) {
    ...
    dimensions = originalPhotos[currentPhotoI].all_tags
      .split(",")
      .filter(function (d) {
        return ignoredDimensions.indexOf(d.split(":")[0]) === -1;
      });
    ...
    if (currentDimension === undefined) {
      console.log(
        "entra al if dentro de updateDimOptions. currentDimension es undefined"
      );
    ...
    if (dimensions.indexOf(currentDimension) === -1) {
      ...
      var dimNameI = dimensions
        .map(function (d) {
          return d.split(":");
        })
        .indexOf(dimension_name);
      if (dimNameI !== -1) {
    ...
    var dimensionNames = d3
      ...
      .values();
    var dimSel = d3
      .select("#dimensionNameSelect")
      .selectAll("option")
      .data(dimensionNames, function (d) {
        return d;
      });

    dimSel.enter().append("option");
    dimSel.text(function (d) {
      return d;
    });
    dimSel.exit().remove();
  ...
  function update(photos, pos) {
    ...
    if (currentPhotoI !== undefined && originalPhotos[currentPhotoI]) {
      ...
      currentDimension =
        currentPhoto.dimension_name + ":" + btoa(currentPhoto.dimension_value); // this fails for non ascii chars. see https://stackoverflow.com/questions/23223718/failed-to-execute-btoa-on-window-the-string-to-be-encoded-contains-characte
    }
    ...
    xScale
      ...
      .range(
        direction === "vertical"
          ? [(w - pageSize) / 2, (w - pageSize) / 2 + pageSize]
          : [0, pageSize]
      );
    ...
    var selPhotos = photosLayer.selectAll(".photo").data(photos, function (d) {
      return "" + d.photo_id + "|" + d.dimension_name + "|" + d.dimension_value;
    });

    var selPhotosEnter = selPhotos
      .enter()
      .append("g")
      .attr("class", "photo")
    ...
    selPhotos.select("image").attr("xlink:href", function (d) {
    ...
    // });
    if (DEBUG) {
      var pages = photosLayer.selectAll(".page").data([0, 1, 2, 3, 4]);
      var pagesEnter = pages.enter().append("g").attr("class", "page");
      ...
      pages
        ...
        .attr("x", function (d) {
          return xScale(0) + (direction === "vertical" ? 0 : pageScale(d));
        })
        .attr("y", function (d) {
          return yScale(0) + (direction === "vertical" ? pageScale(d) : 0);
        })
        //Horizontal
  ...
  function previousDimension() {
    ...
    var nextDimensionIndex =
      (dimensions.indexOf(currentDimension) - 1 + dimensions.length) %
      dimensions.length;
    ...
    console.log(
      "previousDimension currentPhotoI = " +
        currentPhotoI +
        " changingDimensionCount" +
        changingDimensionCount
    );
  ...
  function onClickOnDimensionValue(d) {
    ...
    var currentPhotoID = originalPhotos[currentPhotoI].photo_id;
    console.log(
      "onClickOnDimensionValue currentPhotoI = " +
        currentPhotoI +
        " changingDimensionCount" +
        changingDimensionCount
    );
  ...
  function nextDimensionValue() {
    if (reloading) return;
    reloading = true;
    var values = data.dimension_values.map(function (d) {
      return d.dimension_value;
    });
    var currentValueI = values.indexOf(
      originalPhotos[currentPhotoI].dimension_value
    );
    ...
    console.log(
      "next dimension_value: " +
        data.dimension_values[nextValueI].dimension_value
    );
    currentDimension =
      data.dimension_values[nextValueI].dimension_name +
      ":" +
      data.dimension_values[nextValueI].dimension_value;
  ...
  function jumpToDimensionValue(d) {
    ...
    console.log(
      "jumpToDimensionValue dimension_value: " +
        data.dimension_values[currentValueI].dimension_value
    );
    reloadWithDimensionId(newDimensionID);
  }
  ...
  function updateWithCurrentPhoto() {
    ...
    var dataToShow = originalPhotos.slice(
      Math.max(0, currentPhotoI - Math.floor(NUM_COLS * NUM_ROWS * 2.5)),
      currentPhotoI + Math.floor(NUM_COLS * NUM_ROWS * 2.5)
    );
    dataToShow.forEach(function (d, i) {
      d.pos =
        i +
        (currentPhotoI < 2.5 * NUM_COLS * NUM_ROWS
  ...
  function reloadWithDimensionId(dimension_id, mRedraw) {
    ...
    console.log(
      "reloadWithDimensionId dimension_id=" + dimension_id + " url=" + url
    );
    d3.json(url, callbackPhotos);
  }
  ...
  function reloadWithDimensionIdRight(dimension_id, mRedraw) {
    ...
    url += "&dimension_id=" + dimension_id;
    console.log(
      "reloadWithDimensionId Right dimension_id=" + dimension_id + " url=" + url
    );
    d3.json(url, callbackPhotos);
  }
  ...
  function reloadWithDimensionIdLeft(dimension_id, mRedraw) {
    ...
    url += "?dimension_id=" + dimension_id;
    console.log(
      "reloadWithDimensionIdLeft dimension_id=" + dimension_id + " url=" + url
    );
  ...
  function onDragStart() {
    console.log(
      "Drag start x " + d3.event.sourceEvent.x + " y " + d3.event.sourceEvent.y
    );
  ...
  function onDragEnd() {
    console.log(
      "Drag end x " + d3.event.sourceEvent.x + " y " + d3.event.sourceEvent.y
    );
  ...
  function getMorePhotos(redraw, ID) {
    ...
    ID =
      ID === undefined
        ? originalPhotos[originalPhotos.length - 1].dimension_id
        : ID;
  ...
  function getMorePhotosLeft(redraw, ID) {
    ...
    console.log(
      "getting more photos LEFT redraw=" + redraw + " dimension_id " + ID
    );
